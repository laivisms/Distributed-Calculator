...
public class Worker implements Watcher, Closeable {
	...
	//creates the node under which assignments for this worker are posted by master
	...
	//creates an ephemeral node in the workers folder, notifying the master that this worker now exists and can be used. is ephemeral so that the master will know if the worker has lost connecion,
	//because the ephemeral node will be deleted if it does lose connection
    ...
    synchronized private void updateStatus(String status) {
	    ...
	    //sets the worker status in this worker's ephemeral node, so that anyone who needs to know will know this worker's status
    ...
    void getTasks(){
	//retrieves the tasks assigned to this worker inside its assign node
    ...
    ChildrenCallback tasksGetChildrenCallback = new ChildrenCallback() {
        public void processResult(int rc, String path, Object ctx, List<String> children){
            ...
            case OK:
				...
				//retrieves the request information from the assignment node
    ...
    DataCallback taskDataCallback = new DataCallback() {
		...
		//retry the request
            ...
            case OK:
				...
				//since this task was one part of a calculate task, must input it into the master folder of this task as a partial task completion
                        ...
                        else //this is a full task (insert, delete, or retrieve), so put it under its name in the completed node for the master to pick up
                        	zk.create("/completed/" + taskName, result.getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, taskStatusCreateCallback, ctx);
                        
                        //delete the original node which assigned this request to this worker
